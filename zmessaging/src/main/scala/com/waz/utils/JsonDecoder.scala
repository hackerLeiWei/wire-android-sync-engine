...
object JsonDecoder {
  ...
  implicit def decodeLocalInstant(s: Symbol)(implicit js: JSONObject): LocalInstant = withDefault(s, LocalInstant.Epoch, { js => LocalInstant(Instant.ofEpochMilli(js.getLong(s.name))) })
  implicit def decodeRemoteInstant(s: Symbol)(implicit js: JSONObject): RemoteInstant = withDefault(s, RemoteInstant.Epoch, { js => RemoteInstant(Instant.ofEpochMilli(js.getLong(s.name))) })
  ...
  implicit def decodeOptRemoteInstant(s: Symbol)(implicit js: JSONObject): Option[RemoteInstant] = opt(s, decodeRemoteInstant(s)(_))
