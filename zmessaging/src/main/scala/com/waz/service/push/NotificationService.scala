...
import com.waz.model.GenericContent.{LastRead, MsgDeleted, MsgEdit, MsgRecall, Reaction, Text}
...
trait NotificationUiController {
  /**
    ...
    */
  def onNotificationsChanged(accountId: UserId, ns: Set[NotificationData], silent: Boolean): Future[Unit]
...
class NotificationService(selfUserId:      UserId,
  ...
  val otherDeviceActiveTime = Signal(RemoteInstant.Epoch)
  ...
  val messageNotificationEventsStage = EventScheduler.Stage[ConversationEvent]({ (c, events) =>
    if (events.nonEmpty) {
      events.foreach {
        case GenericMessageEvent(_, time, _, GenericMessage(_, LastRead(_, _))) =>
          otherDeviceActiveTime ! time
        case _ =>
      }
  ...
  private def pushNotificationsToUi(): Future[Unit] = {
    ...
    for {
      ...
      shouldBeSilent <- otherDeviceActiveTime.head.map(t => t.remainingUntil(LocalInstant.apply(clock.instant()).toRemote(drift)) < SilentNotificationsTimeout)
      ...
      _ <- uiController.onNotificationsChanged(selfUserId, filteredShow, shouldBeSilent)
...
object NotificationService {
  ...
  val SilentNotificationsTimeout: FiniteDuration = 1.minute
